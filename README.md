...
Les coefficients de force, de sagesse, de résistance, de magie, de science et la seule partie renvoi du coefficient d'agilité sont ensuite multipliés selon `TOTAL_TP`, chacun par sa propre fonction (`strengthTpMultiplier`, `wisdomTpMultiplier`, `resistanceTpMultiplier`, `magicTpMultiplier`, `scienceTpMultiplier`, `agilityTpMultiplier`) pour rester réglables séparément. Le facteur n'est pas cumulé : le premier palier atteint l'emporte. Chaque barème se lit directement dans `Coefficients.leek`. Aucune distance n'entre dans ces coefficients ; elle reste traitée par le placement final.
