...
Le coefficient de sagesse part d'une base de 0,2, puis ajoute +0,2 au-dessus de 0, +0,3 dès 200, +0,3 dès 400, puis +0,1 dès 500, 600 et 700. Il vaut zéro si `Equipment.healYield` est nul sur poireaux et invocations. Ce rendement compte le soin (`EFFECT_HEAL`, instantané ou par tour) et la vitalité (`EFFECT_BOOST_MAX_LIFE`), les deux seuls effets multipliés par la sagesse, lancés sur un allié ou sur soi ; le soin brut, indépendant de la sagesse, n'y entre pas.

Le coefficient de résistance reprend les paliers de la sagesse, sur la résistance courante. Il vaut zéro si `Equipment.shieldYield` est nul sur poireaux et invocations. Ce rendement compte le bouclier absolu et le bouclier relatif, les deux seuls effets multipliés par la résistance, lancés sur un allié ou sur soi ; les boucliers bruts n'y entrent pas.

Les coefficients de force, de sagesse, de résistance et la seule partie renvoi du coefficient d'agilité sont ensuite multipliés selon `TOTAL_TP`, chacun par sa propre fonction (`strengthTpMultiplier`, `wisdomTpMultiplier`, `resistanceTpMultiplier`, `agilityTpMultiplier`) pour rester réglables séparément. Le facteur n'est pas cumulé : le premier palier atteint l'emporte. Chaque barème se lit directement dans `Coefficients.leek`. Aucune distance n'entre dans ces coefficients ; elle reste traitée par le placement final.
