...
Le coefficient de sagesse suit les mêmes paliers que la force, sur la sagesse courante : base de 0,2, puis +0,2 au-dessus de 0, +0,3 dès 200, +0,3 dès 400, +0,2 dès 600, +0,2 dès 800 et +0,1 dès 1 000. Il vaut zéro si `Equipment.healYield` est nul sur poireaux et invocations. Ce rendement compte le soin (`EFFECT_HEAL`, instantané ou par tour) et la vitalité (`EFFECT_BOOST_MAX_LIFE`), les deux seuls effets multipliés par la sagesse, lancés sur un allié ou sur soi ; le soin brut, indépendant de la sagesse, n'y entre pas.

Le coefficient de force, celui de sagesse et la seule partie renvoi du coefficient d'agilité sont ensuite multipliés selon `TOTAL_TP`, chacun par sa propre fonction (`strengthTpMultiplier`, `wisdomTpMultiplier`, `agilityTpMultiplier`) pour rester réglables séparément. Le facteur n'est pas cumulé : le premier palier atteint l'emporte, par défaut ×0,5 sous 8 PT, ×0,8 dès 8, ×1 dès 16, ×1,5 dès 24 et ×1,7 dès 32. Aucune distance n'entre dans ces coefficients ; elle reste traitée par le placement final.
