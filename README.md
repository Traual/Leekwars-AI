...
Le coefficient de puissance vaut zéro si `Equipment.canUsePower` est faux : aucun effet multiplié par la puissance (dégâts, châtiment `lifeDamageYield`, dégâts nova, poison) n'a de rendement. Ses paliers, ceux de la force, ne portent pas sur la puissance elle-même mais sur la stat qu'elle amplifie : la plus haute entre la force, si l'entité peut frapper, et la magie, si elle peut empoisonner. La puissance n'est pas pondérée par la durée : ses gains sont aujourd'hui permanents.

Les coefficients de force, de sagesse, de résistance, de magie, de science, de puissance et la seule partie renvoi du coefficient d'agilité sont ensuite multipliés selon `TOTAL_TP`, chacun par sa propre fonction (`strengthTpMultiplier`, `wisdomTpMultiplier`, `resistanceTpMultiplier`, `magicTpMultiplier`, `scienceTpMultiplier`, `powerTpMultiplier`, `agilityTpMultiplier`) pour rester réglables séparément. Le facteur n'est pas cumulé : le premier palier atteint l'emporte. Chaque barème se lit directement dans `Coefficients.leek`. Aucune distance n'entre dans ces coefficients ; elle reste traitée par le placement final.
